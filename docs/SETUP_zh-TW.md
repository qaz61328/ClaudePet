...
3. 設定閒聊腳本，偵測可用的 LLM provider（Anthropic API、AWS Bedrock 或 Claude Code CLI）
...
閒聊功能預設關閉，到 Settings 視窗（General 頁籤）開啟。

開啟後，ClaudePet 偵測到閒置狀態（所有 Claude Code session 結束）會啟動計時器（5 分鐘 ± 隨機偏移）。計時器到了就跑外部 shell script，透過 LLM API 產生一句符合角色的閒聊。

腳本解析順序：`Personas/<id>/generate-chatter.sh` → `scripts/generate-chatter.sh`。自動偵測可用 provider：Anthropic API（`ANTHROPIC_API_KEY`）、AWS Bedrock（`aws` CLI）、Claude Code CLI（`claude -p --bare`）。
...
打開 Settings 視窗的「**Keyboard Shortcuts**」頁籤，每個功能一列。點擊錄製區，然後按下你要的修飾鍵＋按鍵組合就好。
...
閒聊功能每次觸發時呼叫 LLM 產出一句短台詞。費用取決於你用的 provider：Anthropic API 用 Claude Haiku、AWS Bedrock 用最便宜的 Claude 模型、Claude Code CLI（`claude -p --bare`）走你現有的 Claude Code 訂閱，token 消耗極少。

想關掉的話，到 Settings 視窗（General 頁籤）取消勾選「Enable Idle Chatter」就好。馬上生效，不用重啟。
...
1. 確認 Settings 視窗（General 頁籤）裡「Enable Idle Chatter」有勾選
